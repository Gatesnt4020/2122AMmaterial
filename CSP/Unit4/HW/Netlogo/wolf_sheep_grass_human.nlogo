...
to setup
  ...
  ifelse model-version != "sheep-wolves" [
    ask patches [
  ...
  ;; marks the end of the ifelse
  [
    ask patches [ set pcolor green ]
  ]
  if model-version = "sheep-wolves-grass-human" [
    create-humans inital-number-humans
    [
    set shape "person"
    set color gray
    set size 2.5
...
to go
  ...
  if not any? wolves or not any? humans and count sheep > max-sheep [ user-message "The sheep have inherited the earth" stop ]
  ask sheep [
    move

    ; in this version, sheep eat grass, grass grows, and it costs sheep energy to move
    if model-version != "sheep-wolves" [
  ...
  if model-version != "sheep-wolves" [ ask patches [ grow-grass ] ]
...
to reproduce-humans
  if random-float 100 < human-reproduction [  ; throw "dice" to see if you will reproduce
    set energy (energy / 2)               ; divide energy between parent and offspring
    hatch 1 [ rt random-float 360 fd 1 ]  ; hatch an offspring and move it forward 1 step
  ]
end
...
to eat-sheep  ; wolf procedure
  ...
  if prey != nobody  [                          ; did we get one? if so,
    ...
    if model-version = "sheep-wolves-grass-humans" [
    if random 2 = 1 [
...
to display-labels
  ...
  if show-energy? [
    ...
    if model-version != "sheep-wolves" [ ask sheep [ set label round energy ]
    if model-version = "sheep-wovles-grass-humans" [ ask humans [ set label round energy ] ] ]
  ]
end
...
"human" 1.0 0 -7500403 true "" "if model-version = \"sheep-wolves-grass-human\" [ plot count humans ]"
...
model-version
model-version
"sheep-wolves" "sheep-wolves-grass" "sheep-wolves-grass-human"
2
