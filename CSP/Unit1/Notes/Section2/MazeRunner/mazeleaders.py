...
def get_names(file_name):
    ...
    return names    #return is making someone else do something and giving it back to you
# return time from the leaderboard file
def get_time(file_name):
    ...
    for line in leaderboard_file:
        ...
        while(line[index]!="\n"):
            ...
            index+=1
        # TODO 5: add the player score to the time list
        time.append(leader_score)

    leaderboard_file.close()
    # TODO 7: return the time in place of the empty list
    return time
# update leaderboard by inserting the current player and score to the list at the correct position
def update_leaderboard(file_name, leader_names, leader_time,  player_name, player_score):
    index = 0
    # TODO 8: loop through all the time in the existing leaderboard list
    while (index < len(leader_time)):
    # TODO 9: check if this is the position to insert new score at
        if (player_score <= int(leader_time[index])):
            break
        else:
            index = index + 1
    ...
    leader_time.insert(index,player_score)
    ...
    while (index < len(leader_names)):
        leaderboard_file.write(leader_names[index] + "," + str(leader_time[index]) + "\n")
        index = index + 1
    leaderboard_file.close()
...
def draw_leaderboard(high_scorer, leader_names, leader_time, turtle_object, player_score):
    ...
    while (index < 5):
        turtle_object.write(str(index + 1) + "\t" + leader_names[index] + "\t" + str(leader_time[index]), font=font_setup)
        turtle_object.penup()
